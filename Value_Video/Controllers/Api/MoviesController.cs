...
namespace Value_Video.Controllers.Api
{
    public class MoviesController : ApiController
    {
        ...
        public IEnumerable<MovieDto> GetMovies(string query = null)
        {
            ...
            {
                 moviesQuery = moviesQuery.Where(item => item.Name.Contains(query)).Where(item => item.NumberInStock > 0);
            }
        ...
        [HttpPut]
        public IHttpActionResult ChangeMovie(int id, MovieDto movieDto)
        {
            if (!ModelState.IsValid)
                return BadRequest();
